...
sub ok_parse() {
	...
	$out_file .= $config->{ 'Star' }->{'schedules_csv'};

	if ( $ret->[0]->{'outf'} ) {
		$out_file = "$ENV{OLIGONYCHUS_HOME}/devel/data/";
		$out_file .= $ret->[0]->{'outf'} . $config->{ 'Star' }->{'schedules_csv'};
	}
...
sub ok_posttask() {
	...
	if ( $ret->[0]->{'outf'} ) {
		$out_file = "$ENV{OLIGONYCHUS_HOME}/devel/data/";
		$out_file .= $ret->[0]->{'outf'} . $config->{ 'Star' }->{'schedules_csv'};
		#$out_file .= 'Star_Movies_10-11-2007_schedules.csv';
	}
	...
	unless ( $posttask->isa('AzTask::StarPostTask') ) {
